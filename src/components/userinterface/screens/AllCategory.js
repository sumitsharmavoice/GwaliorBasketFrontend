            ...
            return (
                <Box
                    ...
                    <Box display="flex" gap={1} fontFamily="arial" textAlign="center" alignItems="center" justifyContent="left" fontSize={matches ? 13 : 15} p={1.5} flexDirection={matches ? "column" : "row"}>
                        ...
                        <p style={{ margin: matches && 0, textDecoration: "none" }}>{item.categorytype}</p>
                    </Box>
                </Box>
            );
        })
...
// redirect page to selected Product Details
const handleExploreProduct=(item)=>{
 navigate(`/exploreproducts`,{state:{data:JSON.stringify(item)}})
        ...
        return products.map((item, i) => {
            return (
                <Grid key={i} item xs={6} md={2.8} style={{ height: matches ? 240 : 300 }}>
                    <Card onClick={()=>handleExploreProduct(item)} sx={{cursor:'pointer', border: "1px solid #ced4d6", height: "100%", width: "100%", boxShadow: "none" }}>
